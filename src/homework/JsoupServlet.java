...
/**
 * Servlet implementation class JsoupServlet
 */
public class JsoupServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int count = dc.queryNewsCount();							// 查询总记录数
		@SuppressWarnings("unchecked")
		List<News> list = dc.queryNews((currPage-1)*News.PAGE_SIZE,News.PAGE_SIZE);			// 查询所有信息
		request.setAttribute("list", list);				// 将list放置到request中
		int pages ; 											// 总页数
		
		if(count % News.PAGE_SIZE == 0){ 					// 计算总页数
			pages = count / News.PAGE_SIZE; 				// 对总页数赋值
		}else{
			pages = count / News.PAGE_SIZE + 1; 				// 对总页数赋值
		}
		...
		for(int i=1; i <= pages; i++){							// 通过循环构建分页导航条
			if(i == currPage){ 								// 判断是否为当前页
				...
				// 构建分页导航条
				sb.append("<a href='./JsoupServlet?page=" + i + "'>" + i + "</a>");
			}
			sb.append("　");									// 构建分页导航条
		}
		request.setAttribute("bar", sb.toString());	// 将分页导航条的字符串放置到request中
		request.getRequestDispatcher("./Programming.jsp").forward(request, response);
  }
}
